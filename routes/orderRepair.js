...
router.post('/addOrderRepair', getBoth,
[
    ...
    const {name, details, location, price} = req.body;
    ...
    const orderRepair = await OrderRepair.create({user, name, details, location, price, status:'Pending'});
...
router.put('/putOrderRepair/:orderRepairId', getAdmin, async (req, res)=>{
    ...
    let orderRepair = await OrderRepair.findByIdAndUpdate(orderRepairId.toString(),{status:status});
